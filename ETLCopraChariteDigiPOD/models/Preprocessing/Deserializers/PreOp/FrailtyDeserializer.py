...
class FrailtyScoreDeserializer(BaseDeserializer):
    def deserialize(self) -> FrailtyScore:
        total_pathological_criteria = self._parse_int_value('QVDELIN030')
        criterion_1 = XMLDeserializerHelper.determine_yes_no_value(self._get_element_value('QVDELIN429'))
        criterion_1_weight = self._parse_decimal_value('QVDELIN430')
        criterion_2 = XMLDeserializerHelper.determine_yes_no_value(self._get_element_value('QVDELIN431'))
        criterion_2_frequency = self._map_value(self._get_element_value('QVDELIN432'), {
            "1": "selten (<1 Tag/Woche)", "2": "gelegentlich (1-2 Tage/Woche)", "3": "häufig (3-4 Tage/Woche)",
            "4": "die meiste Zeit (5-7 Tage/Woche)", "": "k.A."
        })
        criterion_3 = XMLDeserializerHelper.determine_yes_no_value(self._get_element_value('QVDELIN433'))
        criterion_3_metabolism = self._map_value(self._get_element_value('QVDELIN434'), {
        ...
        criterion_4 = XMLDeserializerHelper.determine_yes_no_value(self._get_element_value('QVDELIN435'))
        criterion_4_hand_strength_1 = self._parse_decimal_value('QVDELIN436')
        criterion_4_hand_strength_2 = self._parse_decimal_value('QVDELIN437')
        criterion_4_hand_strength_3 = self._parse_decimal_value('QVDELIN438')
        criterion_4_hand_strength_not_possible = XMLDeserializerHelper.determine_yes_no_value(self._get_element_value('QVDELIN439'))
        criterion_5 = XMLDeserializerHelper.determine_yes_no_value(self._get_element_value('QVDELIN440'))
        criterion_5_seconds = self._parse_decimal_value('QVDELIN441')
        criterion_5_not_possible = XMLDeserializerHelper.determine_yes_no_value(self._get_element_value('QVDELIN442'))
        not_collected = XMLDeserializerHelper.determine_yes_no_value(self._get_element_value('QVDELIN443'))
