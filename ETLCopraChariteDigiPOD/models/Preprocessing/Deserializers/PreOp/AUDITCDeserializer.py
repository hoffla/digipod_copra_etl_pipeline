...
class AlkoholkonsumScoreDeserializer(BaseDeserializer):
    def deserialize(self) -> AlkoholkonsumScore:
        total_score = self._parse_int_value('QVDELIN392')

        question_1 = self._map_value(self._get_element_value('QVDELIN426'), {
            "0": "Nie", "1": "etwa 1 mal pro Monat", "2": "2-4 mal pro Monat", "3": "2-3 mal pro Woche",
            "4": "4 mal oder häufiger pro Woche", "": "k.A."
        })

        question_2 = self._map_value(self._get_element_value('QVDELIN427'), {
            "0": "1 oder 2", "1": "3 oder 4", "2": "5 oder 6", "3": "7 bis 9", "4": "10 oder mehr", "": "k.A."
        })

        question_3 = self._map_value(self._get_element_value('QVDELIN428'), {
            "0": "Nie", "1": "seltener als 1 mal pro Monat", "2": "1 mal pro Monat", "3": "1 mal pro Woche",
            "4": "täglich oder fast täglich", "": "k.A."
        })

        not_collected = XMLDeserializerHelper.determine_yes_no_value(self._get_element_value('QVDELIN444'))

        years_not_drinking = self._parse_int_value('QVDELIN449')
